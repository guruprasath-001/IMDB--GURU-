...
select 'director_mapping'  as ' TABLE ', COUNT(*) AS ' TOTAL ROWS ' from director_mapping
union all
...
select ' ID ' as column_name, count(*) as 'no of null values ' from movie where id is null
union all
...
select year,count(*) as ' TOTAL MOVIES '
FROM movie
group by year ;

select month(date_published) as ' MONTH ',count(*) as 'TOTAL MOVIES '
from movie
